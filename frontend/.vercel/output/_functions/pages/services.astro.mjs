...
</p> </div> <div class="space-y-24"> ${masterplanServices.map((service) => renderTemplate`<div class="service-item"> <div class="mb-12"> ${renderComponent($$result2, "ServiceCard", $$ServiceCard, { "index": service.index, "titleTop": service.titleTop, "titleBottom": service.titleBottom, "img": service.img, "alt": service.alt, "link": service.link })} </div> <div class="grid md:grid-cols-2 gap-10 px-4"> <div> <h3 class="text-2xl font-medium mb-4">Masterplan Guidelines</h3> <p class="mb-6">${service.description}</p> <a${addAttribute(service.link, "href")} class="inline-flex items-center px-8 py-4 bg-green text-dark rounded-2xl hover:bg-dark hover:text-white transition-all font-medium border border-black"> ${service.cta} <svg class="ml-2 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path> </svg> </a> </div> <div class="bg-gray rounded-2xl p-8 border border-black shadow-[0px_3px_0px_#191a23]"> <h3 class="text-2xl font-medium mb-4">Permitted Rights & Rules</h3> <ul class="space-y-4"> ${service.benefits.map((benefit) => renderTemplate`<li class="flex items-start"> <svg class="mr-2 w-5 h-5 mt-1 text-green shrink-0" fill="currentColor" viewBox="0 0 20 20"> <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path> </svg> <span>${benefit}</span> </li>`)} </ul> </div> </div> </div>`)} </div> <div class="mt-24 text-center"> ${renderComponent($$result2, "Card", $$Card, { "isUnderline": true }, { "default": ($$result3) => renderTemplate` <div class="p-10 md:p-16 bg-white rounded-[45px]"> <h2 class="text-3xl md:text-4xl font-medium mb-6">Need to verify a specific plot UPI?</h2> <p class="mb-10 text-xl max-w-2xl mx-auto">
Query our FastAPI backend database or ask our AI assistant for plain-language Kinyarwanda explanations before paying informal land brokers.
</p> <div class="flex justify-center gap-4 flex-wrap"> <a href="/search" class="inline-flex items-center px-10 py-5 bg-dark text-white rounded-2xl hover:bg-green hover:text-dark transition-all font-medium text-xl border border-black">
Verify UPI Number
